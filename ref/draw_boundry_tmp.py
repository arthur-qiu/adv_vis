...
with open('boundry/filelist', 'r') as fid:
    fnames = fid.read().strip().split('\n')
file_path = 'boundry/'
# fnames = ['raw', 'fgsm', 'cw', 'stn']
for fname in fnames:
    full_path = file_path + fname
    points = pk.load(open( full_path ))
    ...
    for model in ['mnist']:
        ...
        plt.savefig('boundry_figs_tmp/{}_hist.png'.format(fname), format='png')    
        for bound in [40, 100, 500, 1000, middle]:
            ...
            plt.savefig('boundry_figs_tmp/{}_{}.png'.format(fname, bound), format='png')
            # plt.show()
